...
for i in {1..3}; do
  rm -f ${data_dir}/diskperftest.txt && time dd bs=100K count=5000 if=/dev/zero of=${data_dir}/diskperftest.txt conv=notrunc oflag=direct,append,noatime
  sleep ${wait_sec}
done
